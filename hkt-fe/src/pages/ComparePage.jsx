...
const ComparePage = () => {
  ...
  const comparisonFeatures = [
    ...
    },
    {
      ...
      render: (p) => (
        <div className="flex items-center justify-center text-yellow-500">
          ...
          <span className="ml-1 text-gray-800">{p.rating || "N/A"}</span>
        </div>
      ),
    },
    ...
    // Mô tả sản phẩm (Hiển thị đầy đủ)
    {
      ...
      label: "Description",
      render: (p) => (
        <div className="text-sm text-left max-w-xs mx-auto p-2 text-gray-700 whitespace-normal">
          {p.description}
        </div>
      ),
    },
        ...
        <h3 className="text-2xl font-bold text-red-600 mb-4">
          {error ||
            (productIds.length === 0
              ...
              : "Products not found or list is empty.")}
        </h3>
        <Link
          to="/"
            ...
            <thead>
              <tr className="bg-gray-100 border-b">
                ...
                {products.map((p, index) => (
                  <th
                    key={p.id}
                    className="px-6 py-4 text-center text-sm font-semibold text-gray-700 w-64 border-l"
                  >
                    <div className="flex justify-center relative">
                      ...
                      >
                        ...
                        <img
                          src={p.imageUrlFront}
                          alt={p.name}
                          className="w-16 h-16 object-contain mx-auto rounded-md mb-2"
                        />
                        {/* Tên sản phẩm */}
                ...
                >
                  ...
                  {products.map((p, colIndex) => (
                    <td
                      ...
                      className="px-6 py-4 text-center border-l w-64 align-top"
                    >
                      {feature.render(p)}
                    </td>
                  ))}
