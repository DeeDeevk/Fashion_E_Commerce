...
const ProductDashboard = ({ onNavigate }) => {
  ...
  const fetchTopProducts = async () => {
    try {
      ...
      setTopProducts(data?.result || data || []);
    } catch (error) {
      setTopProducts([]);
    }
  };
  ...
  const fetchSetData = async () => {
    try {
      ...
      setStatsData((prev) =>
        prev.map((item) => {
          if (item.id === "total")
            return { ...item, value: data.result?.totalProducts || 0 };
          if (item.id === "lowstock")
            return { ...item, value: data.result?.lowStock || 0 };
          return item;
        }),
      );
    } catch (error) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Revenue vs Costs performance over time
              </p>
            </div>
            ...
            <div className="p-4 bg-indigo-50 rounded-2xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-indigo-700/80 leading-relaxed mt-1">
                  Most positive reviews mention "Fast Delivery". Consider
                  highlighting this in your marketing campaigns.
                </p>
              </div>
            </div>
          </div>
